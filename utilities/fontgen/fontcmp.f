...
\ Note: This is way slow, about 1 ms per PEL@.
...
;
: MakeFont  ( -- )
   cr ." Building the font using GetPixel, please wait"
   |usedchars| 0 ?do
      i [usedchars] @ if
        ...
        fheight if  compress
           ...
           ." ."
           column 1+ dup to column  64 > if cr 0 to column then
        else
...
\ Offset to 1st font (24-bit) coarse table
\ ...
...
: SAVEC  ( <filename> -- )              \ save to text file
   parse-word w/o create-file throw to fp
   s" #define FONTBLOB" fp write-file throw
   fhere 0 do
      i 15 and 0= if s"  \" fp write-line throw then
      s" 0x" fp write-file throw
      FontHome i chars + c@ 2 (h.0) fp write-file throw
      i fhere 1- <> if s" ," fp write-file throw then
   loop
   s" " fp write-line throw
   fp close-file throw
;
...
\ Look up a glyph in the font table of FontID, return the address of
\ the bitmap data relative to FontHome.
