...
PANES 8 * CELLS BUFFER: Gpanes  \ separate graphic panes
PAD VALUE PANE          \ Select the current window pane to draw rectangles on

: SetPane       ( n -- )  PANES 1- MIN  8 * CELLS Gpanes + TO PANE ;
: ORGX          ( -- X )  PANE @ ;
: ORGY          ( -- Y )  PANE CELL+ @ ;
...
CREATE EMPTYBOILER
   ...
   56 , 64 ,   1 h, 24 h, 0 ,       \ dims=0,0 24-bit color, no compression
...
\ It would be really nice to copy the HDC_G bitmap into memory.
...
FUNCTION: GetDIBits ( hdc hbm start cLines lpvBits lpbmi usage -- b )

: GETRAWBMP  ( X Y -- )
   LOCALS| Y X |
   hDlg_G @ GetDC  dup to hdc
   CreateCompatibleDC to memDC
   hdc X Y CreateCompatibleBitmap to hBitmap
   memDC hBitmap SelectObject drop
   memDC 0 0 X Y hdc 0 0 SRCCOPY BitBlt drop
   hDC hBitmap 0 Y 0 EzBMP DIB_RGB_COLORS GetDIBits drop
   memDC DeleteDC drop
   hBitmap DeleteObject drop
   hDlg_G @ hdc ReleaseDC drop
;
