...
void AddForthKeywords(struct QuitStruct *state) {
    q = state;
    AddEquate("MAX-N",          "~usage#table:env -- n",           (VM_MASK >> 1));
    AddEquate("MAX-U",          "~usage#table:env -- u",            VM_MASK);
    ...
    AddKeyword("dp!",           "-forth.htm#dpstore a --",          dpStore,    noCompile);
    ...
    AddKeyword(":",             "~core/Colon <name> --",            Definition, noCompile);
    AddKeyword(":noname",       "~core/ColonNONAME -- xt",          NoName,     noCompile);
    AddKeyword(";",             "~core/Semi --",                    SemicoImm,  SemiComp);
    AddKeyword("exit",          "~core/EXIT --",                    noExecute,  CompExit);
    AddKeyword("variable",      "~core/VARIABLE <name> --",         Variable,   noCompile);
    AddKeyword("buffer:",       "~core/BUFFERColon n <name> --",    Buffer,     noCompile);
    AddKeyword("[char]",   "~core/BracketCHAR \"<spaces>name\" --", noExecute,  BrackChar);
    AddKeyword("char",       "~core/CHAR \"<spaces>name\" -- char", Char,       noCompile);
    ...
    AddMacro("3drop",   "-forth.htm#threedrop -- x1 x2 --", (VMO_DROP << 10) | (VMO_DROP << 5) | VMO_DROP);
}
